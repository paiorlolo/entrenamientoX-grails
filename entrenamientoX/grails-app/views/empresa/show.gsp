	...
	<body>
		...
		<div id="show-empresa" class="content scaffold-show" role="main">
			...
			<ol class="property-list empresa">
				...
				<g:if test="${empresaInstance?.equipo}">
				<li class="fieldcontain">
					<span id="equipo-label" class="property-label"><g:message code="empresa.equipo.label" default="Equipo" /></span>
					
						<g:each in="${empresaInstance.equipo}" var="e">
						<span class="property-value" aria-labelledby="equipo-label"><g:link controller="equipo" action="show" id="${e.id}">${e?.encodeAsHTML()}</g:link></span>
						</g:each>
